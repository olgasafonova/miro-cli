...
func newGetCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "get",
		Short: "Get a single frame",
		Long: "Calls GET /v2/boards/{board_id}/frames/{item_id} and prints the\n" +
			"response. Returns title, format, type, showContent, style,\n" +
			"geometry, position, and parent.",
		Args: cobra.NoArgs,
