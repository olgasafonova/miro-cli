...
func newDeleteCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "delete",
		Short: "Delete a frame (destructive)",
		Long: "Calls DELETE /v2/boards/{board_id}/frames/{item_id}.\n\n" +
	...
	cmd.Flags().StringVar(&itemID, "item-id", "", "Frame ID (required)")
...
func runDelete(ctx context.Context, g *clictx.Globals, boardID, itemID string) error {
	...
	path := "/v2/boards/" + boardID + "/frames/" + itemID
	if g.DryRun {
		return g.EmitDryRun("DELETE", path)
	}
	if !g.Yes {
		return &miro.ConfigError{Reason: "refusing to delete frame without --yes; pass --yes to confirm or --dry-run to preview"}
	}
