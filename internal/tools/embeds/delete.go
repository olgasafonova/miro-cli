...
func newDeleteCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "delete",
		Short: "Delete an embed (destructive)",
		Long: "Calls DELETE /v2/boards/{board_id}/embeds/{item_id}.\n\n" +
	...
	cmd.Flags().StringVar(&itemID, "item-id", "", "Embed ID (required)")
...
func runDelete(ctx context.Context, g *clictx.Globals, boardID, itemID string) error {
	...
	path := "/v2/boards/" + boardID + "/embeds/" + itemID
	if g.DryRun {
		return g.EmitDryRun("DELETE", path)
	}
	if !g.Yes {
		return &miro.ConfigError{Reason: "refusing to delete embed without --yes; pass --yes to confirm or --dry-run to preview"}
	}
