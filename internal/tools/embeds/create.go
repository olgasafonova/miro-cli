...
// createFlags captures the per-invocation knobs for `miro embeds create`.
...
func newCreateCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create an embed on a board",
		Long: "Calls POST /v2/boards/{board_id}/embeds with --url (required)\n" +
			"and optional --mode / --preview-url / --x / --y / --width /\n" +
			"--height / --parent-id. Returns the new embed's id and metadata.\n\n" +
			"Coordinates: when --parent-id is set, x/y are relative to the\n" +
			"frame's top-left and the embed's center is placed at (x, y). On\n" +
	...
	cmd.Flags().StringVar(&f.url, "url", "", "Embed source URL (required, e.g. YouTube, Figma, Loom)")
	cmd.Flags().StringVar(&f.mode, "mode", "", "Display mode (inline|modal)")
	cmd.Flags().StringVar(&f.previewURL, "preview-url", "", "Preview image URL")
	...
	cmd.Flags().StringVar(&f.parentID, "parent-id", "", "Frame ID to place the embed inside")
...
func runCreate(ctx context.Context, g *clictx.Globals, f createFlags) error {
	...
	path := "/v2/boards/" + f.boardID + "/embeds"
	if g.DryRun {
...
func buildCreateRequest(f createFlags) createRequest {
	req := createRequest{
		Data: dataField{URL: f.url, Mode: f.mode, PreviewURL: f.previewURL},
	}
