...
func newDeleteCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "delete",
		Short: "Delete an item (destructive)",
		Long: "Calls DELETE /v2/boards/{board_id}/items/{item_id}.\n\n" +
...
func runDelete(ctx context.Context, g *clictx.Globals, boardID, itemID string) error {
	...
	if !g.Yes {
		return &miro.ConfigError{Reason: "refusing to delete item without --yes; pass --yes to confirm or --dry-run to preview"}
	}
