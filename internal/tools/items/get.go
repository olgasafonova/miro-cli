...
func newGetCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "get",
		Short: "Get a single item (any flavour)",
		Long: "Calls GET /v2/boards/{board_id}/items/{item_id}. Returns the\n" +
			"item envelope verbatim; the shape varies by item type (sticky,\n" +
			"shape, text, card, embed, image, frame, etc.).",
		Args: cobra.NoArgs,
