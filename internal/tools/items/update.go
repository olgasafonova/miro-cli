...
//
// Update on the generic items endpoint is narrower than the per-flavour
// endpoints: only position / geometry / parent are mutable. Typed data
// (text content, sticky shape, embed url) goes through the dedicated
// CRUD packages.
...
func newUpdateCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Update an item's position, geometry, or parent (partial)",
		Long: "Calls PATCH /v2/boards/{board_id}/items/{item_id} with only\n" +
			"the fields you set. Pass an empty --parent-id to detach the\n" +
			"item from its frame.\n\n" +
			"This endpoint only mutates position/geometry/parent. To edit\n" +
			"typed data (sticky text, shape style, embed URL) use the\n" +
			"flavour-specific update verbs (stickies update, shapes update,\n" +
			"embeds update, ...).",
		Args: cobra.NoArgs,
	...
	cmd.Flags().StringVar(&f.itemID, "item-id", "", "Item ID (required)")
...
func runUpdate(ctx context.Context, g *clictx.Globals, f updateFlags) error {
	...
	path := "/v2/boards/" + f.boardID + "/items/" + f.itemID
	if g.DryRun {
