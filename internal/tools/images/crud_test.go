...
func TestBuildCreateRequestMinimal(t *testing.T) {
	t.Parallel()
	req := buildCreateRequest(createFlags{url: "https://example.com/image.png"})
	if req.Data.URL != "https://example.com/image.png" {
		t.Errorf("url = %q, want https://example.com/image.png", req.Data.URL)
	}
	if req.Data.Title != "" {
...
func TestBuildCreateRequestFullPayload(t *testing.T) {
	...
	if req.Data.URL != "https://example.com/diagram.png" {
		t.Errorf("url = %q", req.Data.URL)
	}
	if req.Data.Title != "Architecture diagram" {
		t.Errorf("title = %q, want Architecture diagram", req.Data.Title)
	}
...
func TestBuildCreateRequestWidthOnly(t *testing.T) {
	t.Parallel()
	// Miro auto-scales the other dimension; the CLI just passes
	// whatever the user provided.
	req := buildCreateRequest(createFlags{url: "https://example.com/i.png", width: 800})
	if req.Geometry == nil || req.Geometry.Width != 800 {
		t.Errorf("geometry = %+v, want width=800", req.Geometry)
	}
	if req.Geometry.Height != 0 {
		t.Errorf("geometry.height = %v, want 0 (Miro auto-scales)", req.Geometry.Height)
	}
}

func TestBuildCreateRequestHeightOnly(t *testing.T) {
	t.Parallel()
	req := buildCreateRequest(createFlags{url: "https://example.com/i.png", height: 600})
	if req.Geometry == nil || req.Geometry.Height != 600 {
		t.Errorf("geometry = %+v, want height=600", req.Geometry)
	}
	if req.Geometry.Width != 0 {
		t.Errorf("geometry.width = %v, want 0 (Miro auto-scales)", req.Geometry.Width)
	}
}
...
func TestRunCreateSendsBody(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"id":"image-1","data":{"url":"https://example.com/i.png"}}`))
	}))
	defer srv.Close()
	...
	if gotPath != "/v2/boards/uXjV1/images" {
		t.Errorf("path = %q, want /v2/boards/uXjV1/images", gotPath)
	}
	if gotBody.Data.URL != "https://example.com/i.png" {
		t.Errorf("body url = %q, want https://example.com/i.png", gotBody.Data.URL)
	}
	if gotBody.Data.Title != "Test image" {
		t.Errorf("body title = %q, want Test image", gotBody.Data.Title)
	}
	...
	if !strings.Contains(stdout.String(), `"image-1"`) {
		t.Errorf("stdout missing new image id: %q", stdout.String())
	}
}
...
func TestRunCreateRejectsEmptyURL(t *testing.T) {
	...
	err := runCreate(context.Background(), g, createFlags{boardID: "b"})
	if err == nil {
...
func TestRunCreateRejectsEmptyBoardID(t *testing.T) {
	t.Parallel()
	g := &clictx.Globals{Stdout: io.Discard}
	err := runCreate(context.Background(), g, createFlags{url: "https://example.com/i.png"})
	if err == nil {
...
func TestRunCreateDryRunSkipsHTTP(t *testing.T) {
	...
	if err := runCreate(context.Background(), g, createFlags{boardID: "b", url: "https://example.com/i.png"}); err != nil {
		t.Fatalf("runCreate: %v", err)
	}
	if !strings.Contains(stdout.String(), "DRY-RUN POST /v2/boards/b/images") {
...
func TestRunGetHappyPath(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		_, _ = w.Write([]byte(`{"id":"i1","data":{"url":"https://example.com/i.png"}}`))
	}))
	defer srv.Close()
	...
	if gotPath != "/v2/boards/b1/images/i1" {
		t.Errorf("path = %q, want /v2/boards/b1/images/i1", gotPath)
	}
	if !strings.Contains(stdout.String(), `"https://example.com/i.png"`) {
...
func TestBuildUpdateRequestOnlyURLSet(t *testing.T) {
	t.Parallel()
	req, ok := buildUpdateRequest(updateFlags{url: "https://new/i.png", urlSet: true})
	if !ok {
		t.Fatal("buildUpdateRequest with url should report ok=true")
	}
	if req.Data == nil || req.Data.URL != "https://new/i.png" {
		t.Errorf("data.url = %+v, want https://new/i.png", req.Data)
	}
...
func TestBuildUpdateRequestOnlyTitleSet(t *testing.T) {
	t.Parallel()
	req, ok := buildUpdateRequest(updateFlags{title: "New caption", titleSet: true})
	if !ok {
		t.Fatal("buildUpdateRequest with title should report ok=true")
	}
	if req.Data == nil || req.Data.Title != "New caption" {
		t.Errorf("data.title = %+v, want New caption", req.Data)
	}
	if req.Data.URL != "" {
		t.Errorf("data.url should stay empty when --url unset, got %q", req.Data.URL)
	}
}
...
func TestBuildUpdateRequestParentReparent(t *testing.T) {
	t.Parallel()
	req, ok := buildUpdateRequest(updateFlags{parentID: "frame-2", parentIDSet: true})
	if !ok {
		t.Fatal("buildUpdateRequest with parentIDSet should report ok=true")
	}
	if req.Parent == nil || req.Parent.ID != "frame-2" {
		t.Errorf("parent = %+v, want id=frame-2", req.Parent)
	}
}
...
func TestRunUpdatePatchesAndReturnsImage(t *testing.T) {
	t.Parallel()
	var (
		gotMethod string
		gotPath   string
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotPath = r.URL.Path
		_ = json.NewDecoder(r.Body).Decode(&gotBody)
		_, _ = w.Write([]byte(`{"id":"i1","data":{"url":"https://new/i.png"}}`))
	}))
	defer srv.Close()

	var stdout bytes.Buffer
	g := &clictx.Globals{Stdout: &stdout, Client: miro.New(&miro.Config{Token: "t", BaseURL: srv.URL})}
	if err := runUpdate(context.Background(), g, updateFlags{boardID: "b", itemID: "i1", url: "https://new/i.png", urlSet: true}); err != nil {
		t.Fatalf("runUpdate: %v", err)
	}
	if gotMethod != http.MethodPatch {
		t.Errorf("method = %q, want PATCH", gotMethod)
	}
	if gotPath != "/v2/boards/b/images/i1" {
		t.Errorf("path = %q, want /v2/boards/b/images/i1", gotPath)
	}
	if gotBody.Data == nil || gotBody.Data.URL != "https://new/i.png" {
		t.Errorf("body data = %+v, want url=https://new/i.png", gotBody.Data)
	}
	if !strings.Contains(stdout.String(), `"https://new/i.png"`) {
...
func TestRunUpdateRequiresIDs(t *testing.T) {
	t.Parallel()
	g := &clictx.Globals{Stdout: io.Discard}
	if err := runUpdate(context.Background(), g, updateFlags{itemID: "i", url: "x", urlSet: true}); err == nil {
...
func TestRunDeleteDryRunSkipsHTTP(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "DRY-RUN DELETE /v2/boards/b/images/i") {
		t.Errorf("dry-run output: %q", stdout.String())
	}
}
