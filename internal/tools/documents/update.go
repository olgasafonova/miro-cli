...
func newUpdateCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Update a document (partial)",
		Long: "Calls PATCH /v2/boards/{board_id}/documents/{item_id} with only\n" +
			"the fields you set. Pass an empty --parent-id to detach the\n" +
			"document from its frame. --url re-points the document to a new\n" +
			"source.",
		Args: cobra.NoArgs,
	...
	cmd.Flags().StringVar(&f.itemID, "item-id", "", "Document ID (required)")
	cmd.Flags().StringVar(&f.url, "url", "", "New document source URL")
