...
// listFlags captures the per-invocation knobs for `miro groups list`.
...
func newListCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List all groups on a board",
		Long: "Calls GET /v2/boards/{board_id}/groups and prints the cursor-\n" +
			"paginated response. Each group lists its member item IDs.\n\n" +
