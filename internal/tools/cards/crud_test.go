...
func TestBuildCreateRequestMinimal(t *testing.T) {
	t.Parallel()
	req := buildCreateRequest(createFlags{title: "Task 1"})
	if req.Data.Title != "Task 1" {
		t.Errorf("title = %q, want Task 1", req.Data.Title)
	}
	if req.Style != nil {
		t.Errorf("style should be nil when --card-theme unset, got %+v", req.Style)
	}
	if req.Geometry != nil {
		t.Errorf("geometry should be nil when no size flags set, got %+v", req.Geometry)
	}
	if req.Parent != nil {
...
func TestBuildCreateRequestFull(t *testing.T) {
	...
	if req.Data.Title != "Task 1" || req.Data.Description != "Do it" {
		t.Errorf("data = %+v, want title=Task 1 description=Do it", req.Data)
	}
	if req.Data.AssigneeID != "user-1" || req.Data.DueDate != "2026-12-01T00:00:00Z" {
		t.Errorf("data assignee/due = %+v", req.Data)
	}
	if req.Style == nil || req.Style.CardTheme != "#2d9bf0" {
		t.Errorf("style = %+v, want cardTheme=#2d9bf0", req.Style)
	}
	if req.Geometry == nil || req.Geometry.Width != 300 || req.Geometry.Height != 200 {
		t.Errorf("geometry = %+v, want 300x200", req.Geometry)
	}
...
func TestRunCreateSendsBody(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"id":"card-1","data":{"title":"Task 1"}}`))
	}))
	defer srv.Close()

	var stdout bytes.Buffer
	g := &clictx.Globals{Stdout: &stdout, Client: miro.New(&miro.Config{Token: "t", BaseURL: srv.URL})}
	if err := runCreate(context.Background(), g, createFlags{boardID: "uXjV1", title: "Task 1", cardTheme: "#2d9bf0", width: 320, height: 200, parentID: "frame-1"}); err != nil {
		t.Fatalf("runCreate: %v", err)
	}
	if gotMethod != http.MethodPost {
		t.Errorf("method = %q, want POST", gotMethod)
	}
	if gotPath != "/v2/boards/uXjV1/cards" {
		t.Errorf("path = %q, want /v2/boards/uXjV1/cards", gotPath)
	}
	if gotBody.Data.Title != "Task 1" {
		t.Errorf("body title = %q, want Task 1", gotBody.Data.Title)
	}
	if gotBody.Style == nil || gotBody.Style.CardTheme != "#2d9bf0" {
		t.Errorf("body style = %+v, want cardTheme=#2d9bf0", gotBody.Style)
	}
	if gotBody.Geometry == nil || gotBody.Geometry.Width != 320 || gotBody.Geometry.Height != 200 {
		t.Errorf("body geometry = %+v, want 320x200", gotBody.Geometry)
	}
	if gotBody.Parent == nil || gotBody.Parent.ID != "frame-1" {
		t.Errorf("body parent = %+v, want id=frame-1", gotBody.Parent)
	}
	if !strings.Contains(stdout.String(), `"card-1"`) {
		t.Errorf("stdout missing new card id: %q", stdout.String())
	}
}
...
func TestRunCreateRejectsEmptyBoardID(t *testing.T) {
	t.Parallel()
	g := &clictx.Globals{Stdout: io.Discard}
	if err := runCreate(context.Background(), g, createFlags{title: "Task 1"}); err == nil {
...
func TestRunCreateDryRunSkipsHTTP(t *testing.T) {
	...
	if err := runCreate(context.Background(), g, createFlags{boardID: "b", title: "Task 1"}); err != nil {
		t.Fatalf("runCreate: %v", err)
	}
	if !strings.Contains(stdout.String(), "DRY-RUN POST /v2/boards/b/cards") {
...
func TestRunGetHappyPath(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		_, _ = w.Write([]byte(`{"id":"c1","data":{"title":"Task 1"}}`))
	}))
	defer srv.Close()
	...
	if gotPath != "/v2/boards/b1/cards/c1" {
		t.Errorf("path = %q, want /v2/boards/b1/cards/c1", gotPath)
	}
...
func TestBuildUpdateRequestTitleEmptyExplicit(t *testing.T) {
	t.Parallel()
	// --title="" is a valid mutation (clear the title); the bool guard
	// keeps it distinguishable from "didn't pass --title."
	req, ok := buildUpdateRequest(updateFlags{title: "", titleSet: true})
	if !ok {
		t.Fatal("buildUpdateRequest with titleSet should report ok=true even if title empty")
	}
	if req.Data == nil {
		t.Fatal("data should be non-nil when --title set")
	}
	if req.Data.Title != "" {
		t.Errorf("data.title = %q, want empty", req.Data.Title)
	}
}
...
func TestRunUpdatePatchesAndReturnsCard(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotPath = r.URL.Path
		_ = json.NewDecoder(r.Body).Decode(&gotBody)
		_, _ = w.Write([]byte(`{"id":"c1","data":{"title":"new"}}`))
	}))
	defer srv.Close()

	var stdout bytes.Buffer
	g := &clictx.Globals{Stdout: &stdout, Client: miro.New(&miro.Config{Token: "t", BaseURL: srv.URL})}
	if err := runUpdate(context.Background(), g, updateFlags{boardID: "b", itemID: "c1", title: "new", titleSet: true}); err != nil {
		t.Fatalf("runUpdate: %v", err)
	}
	if gotMethod != http.MethodPatch {
		t.Errorf("method = %q, want PATCH", gotMethod)
	}
	if gotPath != "/v2/boards/b/cards/c1" {
		t.Errorf("path = %q, want /v2/boards/b/cards/c1", gotPath)
	}
...
func TestRunUpdateRequiresAtLeastOneField(t *testing.T) {
	t.Parallel()
	g := &clictx.Globals{Stdout: io.Discard}
	if err := runUpdate(context.Background(), g, updateFlags{boardID: "b", itemID: "c"}); err == nil {
...
func TestRunUpdateRequiresIDs(t *testing.T) {
	t.Parallel()
	g := &clictx.Globals{Stdout: io.Discard}
	if err := runUpdate(context.Background(), g, updateFlags{itemID: "c", title: "x", titleSet: true}); err == nil {
...
func TestRunDeleteDryRunSkipsHTTP(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "DRY-RUN DELETE /v2/boards/b/cards/c") {
		t.Errorf("dry-run output: %q", stdout.String())
	}
}
...
func TestNewCmdRegistersAllCRUDVerbs(t *testing.T) {
	...
	for verb, found := range want {
		if !found {
			t.Errorf("`cards` parent missing subcommand %q", verb)
		}
	}
}
