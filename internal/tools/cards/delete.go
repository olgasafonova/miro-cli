...
func newDeleteCmd(g *clictx.Globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "delete",
		Short: "Delete a card (destructive)",
		Long: "Calls DELETE /v2/boards/{board_id}/cards/{item_id}.\n\n" +
	...
	cmd.Flags().StringVar(&itemID, "item-id", "", "Card ID (required)")
...
func runDelete(ctx context.Context, g *clictx.Globals, boardID, itemID string) error {
	...
	path := "/v2/boards/" + boardID + "/cards/" + itemID
	if g.DryRun {
		return g.EmitDryRun("DELETE", path)
	}
	if !g.Yes {
		return &miro.ConfigError{Reason: "refusing to delete card without --yes; pass --yes to confirm or --dry-run to preview"}
	}
