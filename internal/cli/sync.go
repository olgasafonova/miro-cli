...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "embeds":
		return db.UpsertEmbeds(data)
	case "frames":
		return db.UpsertFrames(data)
	case "boards_groups":
		return db.UpsertBoardsGroups(data)
	case "images":
	...
	case "shapes":
		return db.UpsertShapes(data)
	case "sticky_notes":
		return db.UpsertStickyNotes(data)
	case "tags":
		return db.UpsertTags(data)
	case "texts":
		return db.UpsertTexts(data)
	case "groups":
	...
	case "ai_interaction_logs":
		return db.UpsertAiInteractionLogs(data)
	case "orgs_boards":
		return db.UpsertOrgsBoards(data)
	case "cases":
		return db.UpsertCases(data)
	case "content_logs":
		return db.UpsertContentLogs(data)
	case "data_classification_settings":
		return db.UpsertDataClassificationSettings(data)
	case "default_teams_settings":
		return db.UpsertDefaultTeamsSettings(data)
	case "orgs_groups":
		return db.UpsertOrgsGroups(data)
	case "orgs_members":
	...
	case "service_provider_config":
		return db.UpsertServiceProviderConfig(data)
	case "users":
		return db.UpsertUsers(data)
	case "v2_experimental":
		return db.UpsertV2Experimental(data)
	default:
