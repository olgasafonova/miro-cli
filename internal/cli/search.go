...
In local mode: searches locally synced data only.`,
  ...
  miro-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch resourceType {
			case "boards":
				results, err = db.SearchBoards(query, limit)
			case "resource_types":
				results, err = db.SearchResourceTypes(query, limit)
			case "schemas":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchBoards(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search boards failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchResourceTypes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search resource_types failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
