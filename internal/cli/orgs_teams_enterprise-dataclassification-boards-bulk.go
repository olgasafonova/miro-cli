...
func newOrgsTeamsEnterpriseDataclassificationBoardsBulkCmd(flags *rootFlags) *cobra.Command {
	var bodyLabelId int
	var bodyNotClassifiedOnly bool
	var stdinBody bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				if bodyNotClassifiedOnly != false {
					bodyFields["notClassifiedOnly"] = bodyNotClassifiedOnly
				}
				body = bodyFields
			}
	...
	cmd.Flags().IntVar(&bodyLabelId, "label-id", 0, "Data classification label id for team")
	cmd.Flags().BoolVar(&bodyNotClassifiedOnly, "not-classified-only", false, "Assign data classification label to not-classified only or to all boards of team")
